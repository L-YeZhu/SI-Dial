...
class VGDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #print("------check dataloader------", self.filenames[index])
        qa_file = self.filenames[index].replace('datasets','qadata').replace('VG_100K_obj_blur','qas').replace('.jpg','.json')
        #print("---qa file---", qa_file)
        with open(qa_file) as f:
            qa_data = json.load(f)
            questions = qa_data['questions']
            answers = qa_data['answers']
        #print('check q and a', len(questions), len(answers), questions, answers)
        ...
        #print("check VG data", img, target, index, questions, answers)
        return img, target, index, questions, answers
