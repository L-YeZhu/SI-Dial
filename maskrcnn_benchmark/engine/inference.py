...
def compute_on_dataset(model, data_loader, device, synchronize_gather=True, timer=None):
    ...
    sentence_model = SentenceTransformer('multi-qa-mpnet-base-dot-v1')
    for _, batch in enumerate(tqdm(data_loader)):
        with torch.no_grad():
            images, targets, image_ids, questions, answers = batch
            targets = [target.to(device) for target in targets]
            #print("check q and a", questions, answers)
            ...
            if cfg.TEST.BBOX_AUG.ENABLED:
                ...
                output = model(images.to(device), q_emb.to(device), a_emb.to(device), targets)
            if timer:
