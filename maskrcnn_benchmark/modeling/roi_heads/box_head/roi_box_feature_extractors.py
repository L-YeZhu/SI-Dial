...
class FPN2MLPFeatureExtractor(nn.Module):
    ...
    def forward(self, x, proposals):
        #print("-----check roi_feature extractor:", type(x), len(list(x)), list(x)[0].size())
        #print("-----check proposals:", proposals)
        #exit()
        x = self.pooler(x, proposals)
        x = x.view(x.size(0), -1)
        #print('----check x size in box extarctor:', x.size(0))
        x = F.relu(self.fc6(x))
        x = F.relu(self.fc7(x))
    ...
    def forward_without_pool(self, x):
        #print("-----check roi_features extractor w/o pool:", x.size())
        #exit()
        x = x.view(x.size(0), -1)
        x = F.relu(self.fc6(x))
        x = F.relu(self.fc7(x))
