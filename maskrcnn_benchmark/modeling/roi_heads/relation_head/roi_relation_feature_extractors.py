...
class RelationFeatureExtractor(nn.Module):
    ...
    def __init__(self, cfg, in_channels):
        ...
        #self.union_post = nn.Linear(4096+768,4096)
        

    def forward(self, x, proposals, dialog, rel_pair_idxs=None):
        device = x[0].device
        union_proposals = []
        rect_inputs = []
        for proposal, rel_pair_idx in zip(proposals, rel_pair_idxs):
            ...
            #print("check union proposal:", type(union_proposal))
        ...
        if self.cfg.MODEL.ATTRIBUTE_ON:
            ...
            #print("check union_features_att:", union_features_att.size())
            union_features = torch.cat((union_features, union_features_att), dim=-1)
        #print("check x:", type(x))
        #print("check dia_feat:", dialog.size())
        #print("check proposals and union_proposals:", type(proposals),type(union_proposals), len(proposals), len(union_proposals))
        #count = 0
        #for i in range(len(union_proposals)):
        #    num_tmp = getattr(union_proposals[i],"bbox").size()[0]
        #    #print("check union boxes:", num_tmp)
        #    #count += num_tmp
        #    if i == 0:
        #        dia_feat = dialog[i,:,:].repeat(num_tmp,1)
        #    else:
        #        dia_feat_tmp = dialog[i,:,:].repeat(num_tmp,1)
        #        dia_feat = torch.cat((dia_feat, dia_feat_tmp),0)
        #union_features = torch.cat((union_features, dia_feat),1)
        #union_features = self.union_post(union_features)
        #print("check final count:", count)
